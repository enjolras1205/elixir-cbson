...
static inline int read_bson_regex(ErlNifEnv* env, Decoder* d, int st_end,
    ...
    *val = enif_make_new_map(env);
    enif_make_map_put(env, *val, d->atoms->atom_struct, d->atoms->atom_regex,
                      val);
...
static inline ERL_NIF_TERM read_bson_objectid(ErlNifEnv* env, Decoder* d) {
    ...
    if (d->return_json) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            ret_bin[j + 1] = base16charidx[p[i] & 0xf];
        }
        return ret;
    }
    ERL_NIF_TERM ret = enif_make_new_map(env);
    enif_make_map_put(env, ret, d->atoms->atom_struct, d->atoms->atom_objectid,
                      &ret);

    enif_make_map_put(env, ret, d->atoms->atom_objectid_value,
                      enif_make_sub_binary(env, d->arg, d->i, 12), &ret);
    return ret;
}
...
static inline ERL_NIF_TERM read_bson_binary(ErlNifEnv* env, Decoder* d,
    ...
    ERL_NIF_TERM ret = enif_make_new_map(env);
    enif_make_map_put(env, ret, d->atoms->atom_struct, d->atoms->atom_bin,
                      &ret);

    enif_make_map_put(env, ret, d->atoms->atom_bin_value,
                      enif_make_sub_binary(env, d->arg, d->i, len), &ret);

    enif_make_map_put(env, ret, d->atoms->atom_bin_subtype,
                      enif_make_int(env, subtype), &ret);
...
static inline ERL_NIF_TERM read_bson_timestamp(ErlNifEnv* env, Decoder* d,
    ...
    ERL_NIF_TERM ret = enif_make_new_map(env);
    enif_make_map_put(env, ret, d->atoms->atom_struct, d->atoms->atom_timestamp,
                      &ret);
    enif_make_map_put(env, ret, d->atoms->atom_timestamp_value,
                      enif_make_int64(env, timestamp), &ret);
    return ret;
}
...
static inline ERL_NIF_TERM read_bson_utc(ErlNifEnv* env, Decoder* d,
    ...
    ERL_NIF_TERM ret = enif_make_new_map(env);
    enif_make_map_put(env, ret, d->atoms->atom_struct, d->atoms->atom_utc,
                      &ret);
    enif_make_map_put(env, ret, d->atoms->atom_utc_value,
                      enif_make_int64(env, timestamp), &ret);
    return ret;
}
...
ERL_NIF_TERM decode_init(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    while (enif_get_list_cell(env, opts, &val, &opts)) {
        if (get_bytes_per_iter(env, val, &(d->bytes_per_red))) {
            ...
            d->return_atom = 1;
        } else if (enif_compare(val, d->atoms->atom_return_json) == 0) {
            d->return_json = 1;
...
#define ASSERT_LEN(d, l, m)        \
    if ((d)->i + (l) > st_end) {   \
        ret = dec_error((d), (m)); \
        goto done;                 \
    }

#define ASSERT_STRING_LEN(d, l, m)     \
    if ((d)->p[(d)->i + (l)] != 0x0) { \
        ret = dec_error((d), (m));     \
        goto done;                     \
    }
    ...
    next:
        ...
        if (type == 0x0 && st_end == d->i - 1) {
            if (curr_stack == st_doc) {
                if (!make_object(env, curr, &val, d->return_lists,
                                 d->return_atom)) {
            ...
            if (!read_bson_cstring(env, d, &ename, st_end - d->i,
                                   curr_stack == st_array)) {
            ...
            switch (type) {
                ...
                case BSON_REGEX:
                    if (d->return_json) {
                        ret = dec_error(d, "invalid regex to json");
                        goto done;
                    }
...
decode_done:
    if (d->i < bin.size && d->return_trailer) {
        ERL_NIF_TERM trailer =
            enif_make_sub_binary(env, argv[0], d->i, bin.size - d->i);
